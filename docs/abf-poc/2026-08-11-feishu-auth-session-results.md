...
| 完全未登录状态下一键授权并自动读取看板 | Windows 真实本机 CLI + 本地 MCP + 系统默认浏览器 | PASS | 30 秒内 | 无 |
...
1. 授权等待期间人工刷新 Codex 插件，并核对服务端脱敏日志中的会话关联标识保持一致。
2. 人为阻止系统默认浏览器启动，确认真实 CLI 会话进入手动降级。
3. 授权过程中切换 CLI Profile，确认旧会话失败且新 Profile 不继承旧会话身份。
4. 授权成功后立即断网，确认凭据保留、同步失败不撤销连接，并可继续浏览未过期缓存。
