...
2. 校验 marketplace 的 FlowRivet source 指向本次更新的插件源码。
3. `--pull` 时校验工作区干净、存在上游分支，并执行 `git pull --ff-only`。
4. 构建 FlowRivet CLI 和 Codex 插件。
5. 保存 `.codex-plugin/plugin.json` 的原始字节。
6. 使用 Codex 官方 plugin-creator cachebuster helper 临时刷新版本。
7. 执行 `codex plugin add flowrivet@<marketplace>`。
8. 在 `finally` 中恢复 manifest 原始字节。
9. 校验当前 Companion 归属，停止旧实例。
10. 隐藏启动新 Companion，等待 `/health`。
11. 校验已安装插件版本和 Companion 实例。
12. 返回成功结果及“重启 Codex、创建新任务”提示。
...
1. 以字节形式保存原 manifest。
2. 调用官方 `update_plugin_cachebuster.py`。
3. 读取并校验临时版本确实发生变化。
4. 调用 Codex CLI 安装。
5. 无论成功、失败、超时或取消，都在 `finally` 中原子恢复原始内容。
6. 恢复后重新读取并核对字节完全一致。

若进程被操作系统强制终止，下一次更新的 preflight 必须检测 manifest 是否异常，并拒绝继续；不得猜测或自动覆盖用户修改。

更新器不手工编辑 marketplace JSON。它通过 Codex CLI 列表和 marketplace 文件只读解析确认：marketplace 是本地源、插件名为 `flowrivet`、source 最终解析到当前源码目录。
...
三项不能全部确认时停止。迁移只用于旧版本；新实例启动后必须写入实例文件，后续更新不得继续依赖命令行猜测。
...
- PID、端口、product、instance ID 的全部匹配与不匹配组合。
