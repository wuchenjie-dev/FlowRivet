...
10. Codex 分类为研发实现且缺少仓库时，应打开详情抽屉同级的独立仓库模态框；关联成功后发送只含执行 ID 的恢复消息。
11. 账号菜单可以断开飞书项目；断开后清除该 Provider 的活跃缓存并返回登录页。
12. 修改一条已在基线中的飞书任务，等待扫描后右上角铃铛应出现未读数；点击通知应打开飞书原任务。
