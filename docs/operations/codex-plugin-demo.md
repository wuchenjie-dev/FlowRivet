...
8. 点击工作项标题应在 TAPD 打开原始记录；看板本身不改变 TAPD 状态。
...
- 日志只记录 requestId、工具名、Provider、耗时及聚合计数，不记录 Token、身份、项目名、项目 ID、工作项标题、工作项 ID、URL 或响应正文。
...
浏览器测试覆盖桌面和移动端、Token 登录、连接失效、自动项目、真实刷新、部分失败、全部失败、只读卡片和键盘可达性。
